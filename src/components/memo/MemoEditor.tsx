...
import { Memo, Folder } from './MemoView';
...
const MemoEditor: React.FC<MemoEditorProps> = ({ memo, folders, allMemos, onSave, onDelete }) => {
    ...
    useEffect(() => {
        if (memo && (title !== memo.title || content !== memo.content || folderId !== memo.folder_id || JSON.stringify(tags) !== JSON.stringify(memo.tags))) {
            if (autoSaveTimer) {
    ...
    const renderContentWithLinks = (text: string) => {
        ...
        return text.replace(linkRegex, (match, title) => {
            const linkedMemo = allMemos.find(m => m.title.toLowerCase() === title.toLowerCase());
    ...
    const getBacklinks = () => {
        ...
        return allMemos.filter(m =>
            m.id !== memo.id && m.content.includes(`[[${memo.title}]]`)
        );
    };
