...
const EditDialog: React.FC<EditDialogProps> = ({
  ...
  const [notifyDays, setNotifyDays] = useState<string>(
    Math.floor(totalMinutes / 1440).toString(),
  );
  const [notifyHours, setNotifyHours] = useState<string>(
    Math.floor((totalMinutes % 1440) / 60).toString(),
  );
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      notification_minutes:
        calculatedMinutes > 0 ? calculatedMinutes : undefined,
              ...
              {allTasks
                ...
                .map((t) => (
                  ...
                  >
                    ...
                    >
                      {t.group ? `${t.group}: ${t.description}` : t.description}
                    </span>
                  </div>
                ))}
              {allTasks.length <= 1 && (
