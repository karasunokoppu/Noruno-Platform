...
const CalendarView: React.FC<CalendarViewProps> = ({ tasks, groups, onEdit, onAddTask }) => {
    ...
    const handleEventSave = async (eventData: any) => {
        try {
            ...
            if (eventData.id) {
                // Update
                updatedEvents = await invoke<CalendarEvent[]>('update_calendar_event', eventData);
            } else {
                // Create
                updatedEvents = await invoke<CalendarEvent[]>('create_calendar_event', eventData);
            }
    ...
    const handleEventDelete = async (id: string) => {
        ...
        try {
            const updatedEvents = await invoke<CalendarEvent[]>('delete_calendar_event', { id });
            setEvents(updatedEvents);
            setShowEventDialog(false);
            setSelectedEvent(undefined);
        } catch (error) {
    ...
    return (
        <div className="calendar-view">
            ...
            {showTypeSelection && (
                <div className="dialog-overlay" onClick={() => setShowTypeSelection(false)}>
                    <div className="dialog" onClick={e => e.stopPropagation()} style={{ maxWidth: '300px', textAlign: 'center' }}>
            ...
            {showEventDialog && (
                <CalendarEventDialog
                    ...
                    onCancel={() => { setShowEventDialog(false); setSelectedEvent(undefined); }}
                />
            )}
