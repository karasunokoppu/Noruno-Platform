...
**Noruno Platform** は、Tauri v2 + React + TypeScriptで構築されたマルチ機能デスクトップアプリケーションです。タスク管理、高度なメモ、読書記録、カレンダー機能を統合したオールインワンの生産性ツールです。
...
| 機能           | 説明                               |
| -------------- | ---------------------------------- |
| Markdown対応   | ライブプレビュー付きエディター     |
| フォルダー管理 | 階層的なフォルダー構造でメモを整理 |
| タグシステム   | タグによる分類・検索               |
| 検索           | 全文検索                           |
...
CSS Variables (Custom Properties) ベースのテーマシステムを採用。
...
アプリケーションデータは、OS標準のデータディレクトリ内の `noruno_platform` フォルダーにあるJSONファイルに保存されます。

- **Windows**: `C:\Users\[UserName]\AppData\Roaming\com.noruno.platform\`
- **Linux**: `~/.local/share/com.noruno.platform/`
...
- **2024-12**: サブタスク機能、ダッシュボード、Single Instanceプラグイン追加
- **2024-12**: ファイル構造リファクタリング
- **2025-01**: Monokai Dimmedテーマ追加
- **2025-12**:
  - SQLite (SQLx) へのデータベース移行 (一時的対応、後にJSONへ戻しました)
  - テストコードの拡充 (Frontend/Backend)
  - UIスタイリングをTailwind CSSからVanilla CSSへ移行（依存関係削除）
  - カレンダーイベント機能の追加
